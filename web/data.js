...
with te.fp8_autocast(fp8_recipe=recipe):
    ...
    },
    {
        ...
        hpc: false,
        quantization: true,
        description: '6 位浮点格式，3 位指数 + 2 位尾数，Blackwell 架构引入。范围优先变体，适合权重量化。',
        usage: 'LLM 推理量化、权重压缩',
        pros: '比 FP4 精度更高，比 FP8 更省内存',
        cons: '硬件支持有限（Blackwell+）',
        mixedPrecision: '与 FP8/FP16 混合',
        standard: 'NVIDIA Blackwell',
    ...
    },
    {
        ...
        mantissa: 3,
        dynamicRange: '约 10^±1',
        precision: '约 1.5 位十进制',
        training: false,
        inference: true,
        hpc: false,
        quantization: true,
        description: '6 位浮点格式，2 位指数 + 3 位尾数，Blackwell 架构引入。精度优先变体，适合激活值量化。',
        usage: 'LLM 推理量化、激活值压缩',
        pros: '精度高于 E3M2 变体，适合激活值',
        cons: '动态范围小、硬件支持有限（Blackwell+）',
        mixedPrecision: '与 FP8/FP16 混合',
        standard: 'NVIDIA Blackwell',
        hardware: ['nvidia-blackwell'],
        memoryPerParam: 0.75,
    ...
    },
    {
        ...
        hpc: false,
        quantization: true,
        description: 'NVIDIA 专有 FP4 格式（E2M1），配合双级微块缩放（micro-block scaling），Blackwell Tensor Core 原生支持。',
        usage: 'LLM 大规模推理、Blackwell FP4 加速',
        pros: '极致压缩、微块缩放保精度、Blackwell 原生加速',
        cons: 'NVIDIA 专有、需 Blackwell 硬件',
        mixedPrecision: 'FP4 推理 + FP8/FP16 训练',
        standard: 'NVIDIA 专有',
        hardware: ['nvidia-blackwell'],
        memoryPerParam: 0.5,
        relativeSpeed: 7.0,
...
llm = LLM(model="model", quantization="awq")`
    ...
    },
    {
        ...
        hpc: false,
        quantization: true,
        description: '4 位浮点格式（E2M1），保留浮点特性但精度极低。Blackwell 架构 Tensor Core 已原生支持 FP4 计算。',
        usage: 'LLM 量化推理、Blackwell FP4 加速',
        pros: '保留浮点特性、Blackwell 原生加速',
        cons: '精度极低、需量化校准',
        mixedPrecision: 'FP4 推理 + FP8/FP16 训练',
        standard: 'OCP / NVIDIA',
        hardware: ['nvidia-blackwell'],
...
const hardwareSupport = {
    ...
    'fp6_e3m2': {
        'nvidia-volta': 'none', 'nvidia-turing': 'none', 'nvidia-ampere': 'none', 'nvidia-hopper': 'none', 'nvidia-blackwell': 'native',
        'amd-cdna2': 'none', 'amd-cdna3': 'none',
        'intel-xe': 'none', 'intel-gaudi': 'none',
        'google-tpu-v3': 'none', 'google-tpu-v4': 'none', 'google-tpu-v5e': 'none', 'google-tpu-v5p': 'none',
        'huawei-ascend-910': 'none', 'huawei-ascend-910b': 'none', 'huawei-ascend-910c': 'none'
