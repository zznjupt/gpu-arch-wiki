...
- `T`：一行中的 tile 数量，满足 `T = N / b`。
- `x_i`：第 `i` 个 tile 的 score 向量，长度为 `b`。
- `m_i^{loc}`：第 `i` 个 tile 内部的局部最大值。
...
3. 计算 `m_i^{loc} = max_j(x_i[j])`。
4. 更新 `m_i = max(m_{i-1}, m_i^{loc})`。
5. 更新 `d'_i`。
6. 更新 `o'_i`。
...
也可以写成 `o'_{N/b}`，因为 `T = N/b`。

这就是 tiling 版本的重点：`x_i` 不再是一个标量，而是一个 tile 内的 score 向量；`m_i^{loc}`、`d'_i` 和 `o'_i` 把这个 tile 的贡献一次性合并进当前行状态。完整 attention 概率行 `A[k,:]` 不需要物化，只需要在每个 tile 内短暂计算局部指数权重。
